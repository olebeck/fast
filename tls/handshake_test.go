...
var testConfigServer *gotls.Config
...
func runMain(m *testing.M) int {
	...
	testConfigServer = &gotls.Config{
		Time:               func() time.Time { return time.Unix(0, 0) },
		Rand:               zeroSource{},
		InsecureSkipVerify: true,
		CipherSuites:       allCipherSuites(),
		CurvePreferences:   []gotls.CurveID{gotls.X25519, gotls.CurveP256, gotls.CurveP384, gotls.CurveP521},
		MinVersion:         VersionTLS10,
		MaxVersion:         VersionTLS13,
		Certificates:       make([]gotls.Certificate, 2),
	}
	testConfigServer.Certificates[0].Certificate = [][]byte{testRSACertificate}
	testConfigServer.Certificates[0].PrivateKey = testRSAPrivateKey
	testConfigServer.Certificates[1].Certificate = [][]byte{testSNICertificate}
	testConfigServer.Certificates[1].PrivateKey = testRSAPrivateKey
	testConfigServer.BuildNameToCertificate()
	if *keyFile != "" {
...
func testHandshake(t *testing.T, clientConfig *Config, serverConfig *gotls.Config) (serverState gotls.ConnectionState, clientState ConnectionState, err error) {
	...
	server := gotls.Server(s, serverConfig)

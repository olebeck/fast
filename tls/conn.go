...
type Conn struct {
	...
	// input/output
	in, out   halfConn
	rawInput  *bytebufferpool.ByteBuffer // raw input, starting with a record header
	input     *bytebufferpool.ByteBuffer // application data waiting to be read, from rawInput.Next
	hand      *bytebufferpool.ByteBuffer // handshake data waiting to be read
...
func (c *Conn) readRecordOrCCS(expectChangeCipherSpec bool) error {
	...
	hdr := c.rawInput.B[:recordHeaderLen]
	...
	// Process message.
	record := c.rawInput.B[:recordHeaderLen+n]
	c.rawInput.B = c.rawInput.B[len(record):]
	...
	if c.vers == VersionTLS13 && typ != recordTypeHandshake && c.hand != nil {
		return c.in.setErrorLocked(c.sendAlert(alertUnexpectedMessage))
	}
...
func (c *Conn) readHandshake(transcript transcriptHash) (any, error) {
	...
	data = data[:4+n]
	c.hand.B = c.hand.B[len(data):]
	if c.hand.Len() == 0 {
		bytebufferpool.Put(c.hand)
		c.hand = nil
	}
...
func (c *Conn) unmarshalHandshakeMessage(data []byte, transcript transcriptHash) (handshakeMessage, error) {
	...
	//fmt.Printf("unmarshalHandshakeMessage %s %d\n", reflect.TypeOf(m).String(), size.Of(c))
...
func (c *Conn) Read(b []byte) (int, error) {
	...
	if n != 0 && c.input == nil && c.rawInput != nil &&

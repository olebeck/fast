...
func runDynamicRecordSizingTest(t *testing.T, serverConfig *gotls.Config, clientConfig *Config) {
	clientConn, serverConn := localPipe(t)
	serverConfig.DynamicRecordSizingDisabled = false
	tlsConn := gotls.Server(serverConn, serverConfig)
...
/*
	func TestDynamicRecordSizingWithStreamCipher(t *testing.T) {
		...
		runDynamicRecordSizingTest(t, testConfigServer.Clone(), config)
	}
*/
/*
...
func TestHairpinInClose(t *testing.T) {
	...
	tlsConn := gotls.Server(conn, &gotls.Config{
		GetCertificate: func(*gotls.ClientHelloInfo) (*gotls.Certificate, error) {
			panic("unreachable")
		},
	})
...
func TestRecordBadVersionTLS13(t *testing.T) {
	...
	tlsConn := gotls.Server(server, testConfigServer)
	if err := tlsConn.Handshake(); err != nil {
		t.Errorf("Error from client handshake: %v", err)
		return
	}
